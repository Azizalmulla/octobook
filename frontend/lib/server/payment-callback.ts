...
export async function handlePaymentCallback(
  request: Request,
  pathRegistrationId?: string | null,
) {
  try {
    ...
    const registrationId =
      (pathRegistrationId && pathRegistrationId.trim()) ||
      pickFromRecord(queryRecord, REGISTRATION_KEYS) ||
      pickFromRecord(bodyRecord, REGISTRATION_KEYS)
    const trackId = pickFromRecord(queryRecord, TRACK_KEYS) ?? pickFromRecord(bodyRecord, TRACK_KEYS)
    ...
    if (syncedRegistrationId) {
      const returnUrl = new URL(`${getAppUrl()}/return/r/${syncedRegistrationId}`)
